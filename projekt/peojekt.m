...
eq = 0;                 %Värmeförsörjning per volymenhet
...
for i = 1:nelm
   ...
   [f_b_bot, Kce_bot] = findRV(e,t,p,a_air,i,1,T_ref);         %Tar fram randvillkor om elementet vi är på är ett randelement på den "undre" randen
   [f_b_top, Kce_top] = findRV(e,t,p,a_air,i,3,T_inf);         %Tar fram randvillkor om elementet vi är på är ett randelement på den "övre" randen
...
for j = 1:M
    ...
    for i = 1:nelm
        [Ke, fe] = flw2te(ex(i,:),ey(i,:),ep,D,eq);    
        Ce=plantml(ex(i,:),ey(i,:),rho);                                  %Skapar Ce-matrisen från kurshemsidan
        [f_b_bot, Kce_bot] = findRV(e,t,p,a_air,i,1,T_p(tt(j)+dt, T_ref, T_0,t_p));         %Tar fram randvillkor om elementet vi är på är ett randelement på den "undre" randen
        [f_b_top, Kce_top] = findRV(e,t,p,a_air,i,3,T_inf);         %Tar fram randvillkor om elementet vi är på är ett randelement på den "övre" randen
        Ke = Ke + Kce_top + Kce_bot;
        fe = fe + f_b_bot + f_b_top;
        C = assem(edof(i,:),C,Ce);
